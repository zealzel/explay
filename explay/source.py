...
#  class xlManager:
        ...
        #  for each in merger_params:
            ...
            #  if merge_type == "merge_sheets":
                #  df_merged = self.merge_sheets(
                    #  converter_name, location, each["sheet_names"]
                #  )
            ...
            #  elif merge_type == "merge_files":
                #  df_merged = self.merge_files(
                    #  converter_name, location, each["sheet_name"]
                #  )
    ...
    #  def merge_all(
        #  self, converter_name, sheet_name=0, filename_excludes=None, save=True
    #  ):
        #  print(sheet_name, filename_excludes)
        #  source_path = os.path.join(self.home, "source")
        #  self.merger = xlMerger(self.converter, source_path)
        #  df_merged, file_names = self.merger.merge_all(
            #  converter_name, sheet_name, filename_excludes
        #  )
        #  self.sources[converter_name] = df_merged
